...
namespace Cooke.WebSocket
{
    public class WebSocketSession : IDisposable
    {
        ...
        internal WebSocketSession(Socket socket, byte[] inputBuffer, byte[] outputBuffer)
        {
            Socket = socket;
            result = new Queue<string>();
        ...
        public IAsyncResult BeginSendMessage(string message, AsyncCallback asyncCallback, object state = null)
        {
            ...
            var ar = new AsyncResultNoReturnValue(asyncCallback, state);
            Socket.BeginSend(outputBuffer, outputBufferOffset, outputBufferEndOffset - outputBufferOffset, SocketFlags.None, SendMessageCallback, ar);
            return ar;
        }
